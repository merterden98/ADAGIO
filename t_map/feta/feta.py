...
class Feta(ABC):
	...
	def prioritize(self, disease_gene: List[Gene], graph: Union[nx.Graph, None]) -> Set[Tuple[Gene, float]]:
		...
		...
	def __call__(self, disease_gene: Union[Gene, List[Gene]], graph: Union[nx.Graph, None]) -> Set[Tuple[Gene, float]]:
		if isinstance(disease_gene, Gene):
			return self.prioritize([disease_gene], graph)
		elif isinstance(disease_gene, list) and all(isinstance(x, Gene) for x in disease_gene):
			return self.prioritize(disease_gene, graph)
		else:
			Exception("Need to pass a Gene or a list of Genes")
