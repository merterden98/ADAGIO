...
class Glider(PreComputeFeta):
    ...
    def __init__(self, is_annotated=True, lamb: int = 1, is_normalized: bool = False, glide_alph=0.1, glide_beta=1000, glide_delta=1, glide_loc="cw_normalized", k=20) -> None:
        self.__desc = Description(requires_training=False, training_opts=False, hyper_params={
            "lamb": lamb,
    ...
    def prioritize(self, disease_genes: List[Gene], graph: Union[nx.Graph, None]) -> Set[Tuple[Gene, float]]:
        return set(self.final_list(disease_genes, self.__desc.hyper_params["k"]))
