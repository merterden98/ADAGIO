...
class RandomWalkWithRestart(Feta):
	...
	def __init__(self, alpha=0.85):
		self._desc = Description(requires_training=False, training_opts=None, hyper_params={"alpha": alpha})
	def description(self) -> Description:
		return self.__desc
	def prioritize(self, disease_gene: List[Gene], graph: Union[nx.Graph, None]) -> Set[Tuple[Gene, float]]:
		# page rank function in networkx 
		personalization = dict()
		personalization = {gene.name: 1 for gene in disease_gene}
		pr = nx.pagerank(graph, alpha=self._desc.hyper_params["alpha"], personalization=personalization)
		data_set = set()
...
class PreComputeRWR(PreComputeFeta):
	def __init__(self, alpha=0.85):
		self.__desc = Description(requires_training=False, training_opts=None, hyper_params={"alpha": alpha})
	...
	def setup(self, graph: nx.Graph, *args, **kwargs) -> Any:
        ...
        # s = c(I - (1-c)markov)^-1r
		inverse_matrix = np.linalg.inv(I - (1 - prob) * np.asmatrix(markov))
