...
fn main() -> PyResult<()> {
    ...
    let (imports_str, model_str) = set_algo(
...
fn set_data(dataset: &str, hugo: bool, pd: bool) -> (String, String) {
    ...
    imports.push_str(r#"from t_map.hummus.hummus import Hummus
from t_map.hummus.hummus_score import HummusScore, ScoreTypes
"#);
    data.push_str(r#"scoring = HummusScore(score_type=ScoreTypes.TOP_K, k=config['k'])
...
fn set_algo(algorithm: &str, mut imports: String) -> (String, String) {
    ...
    model.push_str(r#"genes = {data.get(i).name for i in range(len(data))}
...
fn configure_and_compile(k_val: &str, alpha_val: &str, import: String, data: String, model: String) -> String {
